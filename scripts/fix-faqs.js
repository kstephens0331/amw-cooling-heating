...
const faqData = {
  ...
  'IndoorAirQuality.jsx': {
    componentName: 'IndoorAirQuality',
    faqs: [
  ...
  'SmartThermostats.jsx': {
    componentName: 'SmartThermostats',
    faqs: [
  ...
  'DryerVentCleaning.jsx': {
    componentName: 'DryerVentCleaning',
    faqs: [
...
for (const [filename, data] of Object.entries(faqData)) {
  ...
  const faqConst = `const serviceFaqs = [\n${faqArrayStr}\n];\n\n`;

  // Insert serviceFaqs array before the component arrow function
  const componentPattern = `const ${data.componentName} = () => {`;
  if (content.includes(componentPattern)) {
    content = content.replace(
      componentPattern,
      faqConst + componentPattern
    );
  } else {
    console.error(`Could not find component pattern in ${filename}: "${componentPattern}"`);
    continue;
  }

  // Add useState hook inside the component (after return opens)
  content = content.replace(
